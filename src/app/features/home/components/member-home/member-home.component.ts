...
export class MemberHomeComponent implements OnInit {
  ...
  readonly isBusinessFeatured = computed(() => {
    return !!this.profile()?.is_featured || !!this.activeFeaturedRequest();
  });

  readonly activeFeaturedRequest = computed(() => {
    const requests = this.dashboardData()?.featuredRequests || [];
    return requests.find((r) => r.status === 'APPROVED') || null;
  });
  ...
  readonly hasActiveFeaturedShowcase = computed(() => {
    return !!this.featuredBannerUrl() && (this.profile()?.is_featured ?? true);
  });
